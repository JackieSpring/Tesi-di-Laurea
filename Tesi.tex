...
<meta http-equiv="Content-Security-Policy" content="script-src unsafe-inline" />
    ...
    <object data="javascript:location.href='about:blank'">
        ...
        \label{sec:attaccando-vuln-bypass-iframe}
            Un tag \iframe rappresenta un \todo{Aggiungere Browsing e Window context al capitolo sicurezza}Browsing Context
...
<iframe srcdoc="<html><body><script src=http://localhost/code.js ><\/script></body></html>">
    ...
    <html lang="en">
        ...
        \subsection{Esecuzione di script da origini fidate}
            Usando l'attributo \code{srcdoc} è stato possibile usare gli \iframe per eseguire un tag \script nella
            pagina dell'estensione, ma non è stato possibile caricare codice arbitrario a causa delle 
            restrizioni di CSP che impediscono l'ottenimento di una risorsa \JS da origini remote o inline
            ma non da origini locali. Sotto le giuste condizioni le fonti locali potrebbero essere
            sufficienti per compromettere il sistema della vittima. Per dimostrare quanto detto però
            è necessario fare una piccola modifica ai codici di \vuln{}.\\
            Introduco il file \file{sidebar/win.js} nel percorso della sidebar, il \refCode{code:win-js-example-1}
            ne mostra il contenuto.
            \begin{lstlisting}[label=code:win-js-example-1]
                parent.document.body.innerHTML = "YOU WON"
            \end{lstlisting}
            Si tratta di un semplice defacciamento della pagina genitore, ai fini dell'esperimento si immagini
            che \file{sidebar/win.js} contenga 
            \begin{lstlisting}[label=code:iframe-win-js-example-1]
                //
            \end{lstlisting}
